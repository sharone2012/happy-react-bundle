...
export default function CFI_OPDC_DM_Calc() {
  ...
  const _calcData  = useSelector(function(s) { return s.calc.results['opdc-dm'] || null; });
  const loading    = useSelector(function(s) { return s.calc.loading['opdc-dm'] || false; });

  useEffect(function() {
    dispatch(runCalc({ name: 'opdc-dm', body: { ffb, util, hrs, days, yieldPct, mc, lockedMC } }));
  }, [ffb, util, hrs, days, yieldPct, mc, lockedMC, dispatch]);

  var effFFB      = (_calcData && _calcData.effFFB)      || 0;
  var ffbDay      = (_calcData && _calcData.ffbDay)      || 0;
  var ffbMonth    = (_calcData && _calcData.ffbMonth)    || 0;
  var opdcFWhr    = (_calcData && _calcData.opdcFWhr)    || 0;
  var opdcFWday   = (_calcData && _calcData.opdcFWday)   || 0;
  var opdcFWmonth = (_calcData && _calcData.opdcFWmonth) || 0;
  var opdcDMhr    = (_calcData && _calcData.opdcDMhr)    || 0;
  var opdcDMday   = (_calcData && _calcData.opdcDMday)   || 0;
  var opdcDMmonth = (_calcData && _calcData.opdcDMmonth) || 0;
  var waterDay    = (_calcData && _calcData.waterDay)    || 0;
  var bsfReqDMday = (_calcData && _calcData.bsfReqDMday) || 0;
  var shortfallDM = (_calcData && _calcData.shortfallDM) || 0;
  var selfSuffPct = (_calcData && _calcData.selfSuffPct) || 0;
  var nDay        = (_calcData && _calcData.nDay)        || 0;
  var pDay        = (_calcData && _calcData.pDay)        || 0;
  var kDay        = (_calcData && _calcData.kDay)        || 0;
  var caDay       = (_calcData && _calcData.caDay)       || 0;
  var mgDay       = (_calcData && _calcData.mgDay)       || 0;
  var ligninDay   = (_calcData && _calcData.ligninDay)   || 0;
  var proteinDay  = (_calcData && _calcData.proteinDay)  || 0;
  var efbFWday    = (_calcData && _calcData.efbFWday)    || 0;
  var opdcPctEFB  = (_calcData && _calcData.opdcPctEFB)  || 0;
  var rows        = (_calcData && _calcData.rows)        || [];
  var lockedRow   = (_calcData && _calcData.lockedRow)   || { mc: mc, fw: 0, dm: 0, water: 0, pctFFB: 0, flag: false };
  ...
  return React.createElement("div", {
    ...
    React.createElement("div", { style:{ maxWidth:940, margin:"0 auto", padding:"16px 14px" } },
      ...
      loading ? React.createElement("div", { style:{ display:"flex", alignItems:"center", justifyContent:"center", gap:8, padding:"8px 14px", background:C.amber+"11", borderBottom:"1px solid "+C.amber+"33", color:C.amber, fontSize:12, letterSpacing:"0.05em", marginBottom:8 } },
        React.createElement("div", { style:{ width:14, height:14, border:"2px solid "+C.amber+"55", borderTopColor:C.amber, borderRadius:"50%", animation:"cfi-spin 0.7s linear infinite" } }),
        "Calculating\u2026"
      ) : null,
