...
export default function CFI_POME_DM_v3() {
  ...
  const _calcData  = useSelector(function(s) { return s.calc.results['pos-dm'] || null; });
  const loading    = useSelector(function(s) { return s.calc.loading['pos-dm'] || false; });

  useEffect(function() {
    dispatch(runCalc({ name: 'pos-dm', endpoint: 'pome-dm', body: { ffb, util, hrs, days, scenario, dmPoint, lockedMC, dec2Active, dec2TSSin, dec2TSSrem, dec2CakeMC } }));
  }, [ffb, util, hrs, days, scenario, dmPoint, lockedMC, dec2Active, dec2TSSin, dec2TSSrem, dec2CakeMC, dispatch]);
  ...
  var pomeDMday      = (_calcData && _calcData.pomeDMday)      || 0;
  var pomeDMmonth    = (_calcData && _calcData.pomeDMmonth)    || 0;
  var pomeM3hr       = (_calcData && _calcData.pomeM3hr)       || 0;
  var pomeM3day      = (_calcData && _calcData.pomeM3day)      || 0;
  var biogasDay      = (_calcData && _calcData.biogasDay)      || 0;
  var ch4Day         = (_calcData && _calcData.ch4Day)         || 0;
  var ghgAvoidedDay  = (_calcData && _calcData.ghgAvoidedDay)  || 0;
  var dec2WetSolids  = (_calcData && _calcData.dec2WetSolids)  || 0;
  var dec2DM         = (_calcData && _calcData.dec2DM)         || 0;
  var totalPomeDMday   = (_calcData && _calcData.totalPomeDMday)   || 0;
  var totalPomeDMmonth = (_calcData && _calcData.totalPomeDMmonth) || 0;
  var rows      = (_calcData && _calcData.rows)      || [];
  var lockedRow = (_calcData && _calcData.lockedRow) || { mc: lockedMC, fw: 0, water: 0, pctFFB: 0 };
  var rangeRows = (_calcData && _calcData.rangeRows) || [];
  ...
  return React.createElement("div", {
    ...
    React.createElement("div", { style:{ maxWidth:940, margin:"0 auto", padding:"16px 14px" } },
      ...
      loading && React.createElement("div", { style:{ display:"flex", alignItems:"center", gap:10, padding:"10px 0", marginBottom:8 } },
        React.createElement("div", { style:{ width:18, height:18, borderRadius:"50%", border:"3px solid "+C.teal+"44", borderTopColor:C.teal, animation:"cfi-spin 0.7s linear infinite" } }),
        React.createElement("span", { style:{ color:C.grey, fontSize:12 } }, "Calculating\u2026")
      ),
