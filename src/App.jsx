  ...
  return (
    ...
    open && React.createElement("div", {
      style: {
        ...
        background: "#0B1929", border: "1px solid " + dotColor + "66",
    ...
    React.createElement("div", { style: { fontSize: 9, color: "#a0b8d0", fontFamily: "monospace", lineHeight: 1.5, background: "#0f2236", borderRadius: 4, padding: "4px 6px" } }, logic)
    ),
...
// Math: weighted C:N = Σ(Ni × CNi) / Σ(Ni); CP% = N% × 6.25
...
function BlendOptimiser({ efbDMpd, opdcDMreq, pomeSludgeMaxPct, pomeSludgeNatTPD,
  ...
  const baseN = efbDMpd * 7.6 + opdcDMreq * 23.2; // kg N/day
  const baseC = efbDMpd * 7.6 * 60 + opdcDMreq * 23.2 * 20; // C-numerator

  // ── STREAM CONSTANTS ──
  // POME: N=17.6 kg/t DM, C:N=15  → each t DM adds 17.6 kg N and 264 to C-numerator
  // PKE:  N=29.0 kg/t DM, C:N=15  → each t DM adds 29.0 kg N and 435 to C-numerator
  const POME_N = 17.6,POME_CN = 15,POME_C = POME_N * POME_CN; // 264
  const PKE_N = 29.0,PKE_CN = 15,PKE_C = PKE_N * PKE_CN; // 435
  ...
  const solve = useMemo(() => {
    ...
    const calc = (x, y) => {
      ...
      const cpPct = totDM > 0 ? totN / (totDM * 10) * 6.25 : null;
    ...
    // ── SINGLE TARGET CP% ──
    // cp_r = T_cp/6.25/100 × 1000 = T_cp × 1.6  (kg N per t DM target)
    const solveCPpome = (T) => {
      const r = T * 1.6;
    ...
    if (mode === "pome" && hasCN) {
      ...
      if (x === null) {result = { error: "No solution (POME C:N=15 cannot solve this target)" };} else
      if (x < 0) {result = { error: `C:N already ≤ target. Current: ${currentCN}` };} else
      {
      ...
      if (y === null) {result = { error: "No solution — PKE C:N=15, same as POME; cannot lower C:N further than PKE allows." };} else
      if (y < 0) {result = { error: `C:N already ≤ target. Current: ${currentCN}` };} else
      {result = calc(0, y);strategy = "PKE only";}
    } else if (mode === "dual" && hasCN && hasCP) {
      const sol = solveDual(T_cn, T_cp);
      if (!sol) {result = { error: "System has no unique solution — targets may be contradictory." };} else
      {
        const { x, y } = sol;
        const capped = x > pomeMaxDM;
        if (capped) {
      ...
      const x = solveCN_pome(T_cn);
      if (x === null || x < 0) {result = { error: x === null ? "No POME solution." : `C:N already ≤ target. Current: ${currentCN}` };} else
      {
        const capped = x > pomeMaxDM;
        const xUse = Math.min(x, pomeMaxDM);
        if (capped) {
  ...
  return (
    <div style={{ ...S.card, border: `1px solid ${C.purple}55`, gridColumn: "1/-1" }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5, marginTop: 2 }}>
            ...
            style={{ padding: "5px 10px", borderRadius: 5, cursor: "pointer", fontSize: 11,
              background: mode === opt.v ? opt.c + "22" : C.navyLt,
              border: `1px solid ${mode === opt.v ? opt.c : C.navyLt}`,
      ...
      {solve && !solve.error &&
      <div>
          <div style={{ background: C.navyLt, borderRadius: 8, padding: 14,
          border: `1px solid ${C.purple}44`, marginBottom: 10 }}>
            ...
            map((k, i) =>
            <div key={i} style={{ background: C.navy, borderRadius: 6, padding: "8px 10px", textAlign: "center" }}>
      ...
      <div style={{ marginTop: 12, background: C.navyDk, borderRadius: 6, padding: 10 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 6 }}>
          {[
          { s: "EFB", n: "7.6 kg/t DM", cn: "60", c: C.teal },
          { s: "OPDC", n: "23.2 kg/t DM", cn: "20", c: C.amber },
          { s: "POME Sludge", n: "17.6 kg/t DM", cn: "15", c: C.blue },
          { s: "PKE", n: "29.0 kg/t DM", cn: "15", c: C.purple }].
          map((row, i) =>
          <div key={i} style={{ background: C.navyMid, borderRadius: 4, padding: "6px 8px",
...
function NutrLedger({ stg, N, P, K, Ca, Mg, OM, cn, wetPD, mc, nAdj, pAdj, ag, col }) {
  ...
  return (
    <div style={{ background: C.navyLt, border: `1px solid ${col || C.teal}44`, borderRadius: 10, padding: 14, marginTop: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 6, marginBottom: 10 }}>
        <div style={{ color: col || C.teal, fontWeight: 700, fontSize: 12, letterSpacing: 1 }}>⚗️ NUTRIENT LEDGER — {stg}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr", gap: 14 }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
          <thead><tr>{["Nutrient", "% DM", "kg/day", "$/t FW", "$/day"].map((h) =>
              <th key={h} style={{ borderBottom: `1px solid ${C.navyDk}`, color: C.grey, textAlign: h === "Nutrient" ? "left" : "right", padding: "3px 6px", fontWeight: 600, fontSize: 10 }}>{h}</th>
              )}</tr></thead>
          <tbody>
            {[
            ...
            map((r, i) =>
            <tr key={i} style={{ borderBottom: `1px solid ${C.navyDk}44` }}>
        ...
        <div style={{ background: C.navy, borderRadius: 8, padding: 12 }}>
          ...
          map((r, i) =>
          <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "4px 0", borderBottom: `1px solid ${C.navyLt}55`, fontSize: 10 }}>
...
export default function CFI() {
  ...
  const carbon_opdcDMmo = +(monthFFB * 0.0456 * (1 - s0.opdcMC / 100) * (s0.opdcCapturePct / 100)).toFixed(1);
  ...
  const pkeNpd = +(pkeDMpd * 29.0).toFixed(1);
  const pkeCostDay = +(s0.pkeTPD * 160).toFixed(0);

  // ── COMBINED 3-STREAM NPK (kg/day) ───────────────────────────────────────
  const efbN_kgpd = +(efbDMpd * 7.6).toFixed(1);
  const efbP_kgpd = +(efbDMpd * 0.6).toFixed(1);
  const efbK_kgpd = +(efbDMpd * 7.4).toFixed(1);
  const opdcN_kgpd = +(opdcDMreq * 23.2).toFixed(1);
  const opdcP_kgpd = +(opdcDMreq * 3.0).toFixed(1);
  const opdcK_kgpd = +(opdcDMreq * 7.5).toFixed(1);
  ...
  const blendCN_efb = efbDMpd * 7.6 * 60;
  const blendCN_opdc = opdcDMreq * 23.2 * 20;
  const blendCN_pome = pomeSludgeInclDMpd * 17.6 * 15;
  const blendCN_pke = pkeDMpd * 29.0 * 15;
  ...
  const s2Conflicts = [];
  if (s2.naoh) s2Conflicts.push({ id: "naoh_alert", sev: "red", msg: "NaOH active — Na⁺ residues toxic to BSF above 0.3% DM. Mandatory rinse cycle required before BSF introduction. Monitor substrate Na after neutralisation." });
  ...
  const s3Conflicts = [];
  if (s3orgs.bt_icbb) s3Conflicts.push({ id: "bt_bsf", sev: "amber", msg: "⚠️ Bt (Bacillus thuringiensis) ICBB 6095 IS ACTIVE — ALLOWED but USE WITH CAUTION. Cry1A + Cry3A proteins are toxic to Diptera larvae at high concentrations. Recommended: Apply Bt only during S3 composting phase (Wave 1/2) BEFORE BSF introduction. Confirm Bt titre has degraded below 10⁴ CFU/g before S4 loading. Monitor larvae for early mortality signs (Day 1–3)." });
  ...
  const substrateCP_S3 = +(blendCN ? (100 / (blendCN * 6.25) * 100).toFixed(1) : 8.4); // crude protein from C:N
  ...
  const GateRow = ({ n, name, target, measured, pass, fail_action }) => {
    ...
    return (
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px",
        background: hasVal ? pass ? C.green + "18" : C.red + "18" : C.navyLt,
        borderRadius: 6, marginBottom: 4,
        border: `1px solid ${hasVal ? pass ? C.green + "44" : C.red + "44" : C.navyLt}` }}>
    ...
    <div style={{ background: C.navy, minHeight: "100vh", fontFamily: "'DM Sans', system-ui, sans-serif",
      ...
      <div style={{ padding: "20px 24px", maxWidth: 1100, margin: "0 auto" }}>
        ...
        {stage === 0 && (() => {
          ...
          const opdcN = opdcN_kgpd,opdcP = opdcP_kgpd,opdcK = opdcK_kgpd;
          const totN = totalN_kgpd,totP = totalP_kgpd,totK = totalK_kgpd;
          const pomeSludgeInclDM = pomeSludgeInclDMpd;
          const cprice = carbonPriceActive;
          const co2est = co2e100_total_mo;
          const co2annual = co2e100_annual;
          const carbRev = carbon_rev_annual;
            ...
            <div style={{ background: C.navyDk, borderBottom: `1px solid ${C.teal}22`,
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ background: C.teal + "22", border: `1px solid ${C.teal}55`,
                    borderRadius: 8, padding: "5px 12px", color: C.teal, fontWeight: 900, fontSize: 13, letterSpacing: "0.1em" }}>S0</div>
            ...
            <div style={{ display: "flex", gap: 16, alignItems: "start", marginBottom: 16 }}>
                ...
                <Card style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "stretch", gap: 0, marginTop: 12 }}>
                    <div style={{ background: C.amber + "22", border: `1px solid ${C.amber}55`, borderRadius: "6px 0 0 6px", padding: "10px 10px", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center" }}>
                    ...
                    <div style={{ background: C.amber + "22", border: `1px solid ${C.amber}55`, borderLeft: "none", borderRadius: "0 6px 6px 0", padding: "10px 14px", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center" }}>
                      <span style={{ color: C.amber, fontSize: 18, fontWeight: 700 }}>{efbMonthWet.toLocaleString()}</span>
                      <div style={{ color: C.amber, fontSize: 9, fontWeight: 600, textTransform: "uppercase", opacity: 0.85 }}>Fresh Weight</div>
                    </div>
                  </div>
                </Card>
            </div>
              ...
              <Card>
                ...
                <Divider />
                  ...
                  <div style={{ background: C.navyDk, borderRadius: 8, padding: "10px 12px" }}>
                    ...
                    <div style={{ color: C.grey, fontSize: 9 }}>
                      = {pomeWetReq} t FW/day @ {pomeSludgeActMC}% MC
                    </div>
                    <div style={{ color: pomeSupplyOK ? C.green : C.red, fontSize: 9, fontWeight: 700, marginTop: 4 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 16 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <div key={chem.k} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "8px 10px",
                  background: s2[chem.k] ? chem.col + "14" : C.navyLt, borderRadius: 6, marginBottom: 4,
                ...
                map((chem) =>
                <div key={chem.k} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "8px 10px",
                  background: s2[chem.k] ? C.purple + "14" : C.navyLt, borderRadius: 6, marginBottom: 4,
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <div key={r.n} style={{ display: "grid", gridTemplateColumns: "20px 1fr 80px", gap: 6,
                  padding: "6px 8px", background: C.navyLt, borderRadius: 4, marginBottom: 3 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 360px", gap: 16 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <div key={o.k} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "9px 12px",
                  background: s3orgs[o.k] ? o.col + "14" : C.navyLt, borderRadius: 7, marginBottom: 5,
                  border: s3orgs[o.k] ? `1px solid ${o.col}44` : "none", borderLeft: `3px solid ${s3orgs[o.k] ? o.col : C.navyLt}` }}>
                  ...
                  <div style={{ background: C.teal + "22", border: `1px solid ${C.teal}44`, borderRadius: 6,
                ...
                <div key={o.k} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "8px 10px",
                  background: s3orgs[o.k] ? o.col + "14" : C.navyLt, borderRadius: 6, marginBottom: 4,
                ...
                map((o) =>
                <div key={o.k} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "8px 10px",
                  background: s3orgs[o.k] ? C.purple + "14" : C.navyLt, borderRadius: 6, marginBottom: 4,
                ...
                <div style={{ background: C.amber + "18", border: `2px solid ${C.amber}77`, borderRadius: 8, padding: 14 }}>
                  <div style={{ color: C.amber, fontWeight: 900, fontSize: 12, marginBottom: 6 }}>⚠️ ALLOWED — TIMING-CRITICAL / USE WITH CAUTION IN BSF PATHWAYS</div>
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "8px 10px", background: C.amber + "22", borderRadius: 6 }}>
                    ...
                    <div>
                      ...
                      <div style={{ color: C.greyLt, fontSize: 10, marginTop: 3 }}>⚠️ Produces Cry1A + Cry3A proteins which at high titres are toxic to Diptera larvae (BSF). <strong style={{ color: C.amber }}>ALLOWED</strong> — apply during S3 composting phase only (Wave 1/2). Must confirm Bt titre has degraded to &lt;10⁴ CFU/g before S4 BSF loading. Monitor larvae Day 1–3 for mortality signs. Do NOT apply directly to active BSF rearing beds.</div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                { label: "pH (S2 out)", pass: phPass, val: phGate != null ? `${typeof phGate === "number" ? phGate.toFixed(1) : phGate} — target 6.5–8.0 ${s3.ph_measured ? "[measured]" : "[PKSA assumed]"}` : "Enter pH measurement", icon: "⚗️" },
                { label: "Temperature", pass: tmpPass, val: tmpGate != null ? `${tmpGate}°C — target ≤30°C [measured]` : "Enter substrate temp °C", icon: "🌡️" },
                { label: "Bt status", pass: btPass, val: btPass ? "Bt ICBB 6095 not active ✅" : "⚠️ Bt ACTIVE — Confirm titre <10⁴ CFU/g & 48h >55°C before S4", icon: "🧬" },
                { label: "Visual OK", pass: visPass, val: "Confirm substrate not compacted / anaerobic", icon: "👁️" }].
                map((g, i) =>
                ...
                map((r, i) =>
                <div key={i} style={{ padding: "8px 10px", background: C.navyLt, borderRadius: 5, marginBottom: 4,
        ...
        <div>
            ...
            <div key={id} onClick={() => upS4("s4tab", id)} style={{
              ...
              background: s4.s4tab === id ? C.teal + "33" : C.navyLt,
              border: `1px solid ${s4.s4tab === id ? C.teal : C.navyLt}`,
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                ...
                map((r, i) =>
                <div key={i} style={{ background: C.navyLt, borderRadius: 5, padding: "7px 10px" }}>
                ...
                <div style={{ ...S.card, border: `1px solid ${C.amber}44` }}>
                  ...
                  <div style={{ padding: "8px 12px", background: C.navyLt, borderRadius: 6, marginBottom: 8 }}>
                ...
                <div style={{ ...S.card, gridColumn: "1/-1" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10, marginBottom: 12 }}>
                    ...
                    <div style={{ background: C.navyLt, borderRadius: 6, padding: "10px 14px", borderTop: `3px solid ${C.green}` }}>
                    ...
                    <div style={{ background: C.navyLt, borderRadius: 6, padding: "10px 14px", borderTop: `3px solid ${C.amber}` }}>
                    ...
                    <div style={{ background: C.navyLt, borderRadius: 6, padding: "10px 14px", borderTop: `3px solid ${C.blue}` }}>
                  ...
                  <div style={{ background: C.navyLt, borderRadius: 6, padding: "10px 14px", marginBottom: 10 }}>
                ...
                <div key={opt.id} onClick={() => upS4("pathwayS5", opt.id)} style={{
                  background: s4.pathwayS5 === opt.id ? opt.color + "22" : C.navyLt,
                  ...
                  borderRadius: 8, padding: "14px 16px", cursor: "pointer" }}>
                    ...
                    <div key={l} style={{ background: C.navy, borderRadius: 4, padding: "5px 8px", textAlign: "center" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                ...
                <div style={S.card}>
                  ...
                  <div style={{ marginBottom: 12 }}>
                    ...
                    background: s4.pkmEnabled ? C.green : C.navyLt,
              ...
              <div>
                ...
                style={{ width: "100%", accentColor: C.amber }} />
                  ...
                  <hr style={S.divider} />
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    <div style={{ background: C.navyLt, borderRadius: 5, padding: "8px 12px" }}>
                  ...
                  {pkmWarning &&
              <Alert type="error" msg={`⚠️ CRITICAL: Substrate CP ${adjustedCP}% DM is below 8% minimum for BSF growth. EFB-only substrate at 3.4% CP causes catastrophic larval failure. Enable PKM supplementation at ≥30% DM or increase OPDC blending.`} />
              }
              ...
              <Alert type="warn" msg={`CP ${adjustedCP}% DM — acceptable but sub-optimal. Larvae viable but growth rate may be 15–25% below potential. Consider increasing PKM to reach 12% DM.`} />
                ...
                ["Dry Matter", "~88%"],
                ["BSF safety", "⚠️ Allowed — apply S3 phase only; confirm titre decay before S4"],
                ...
                map(([range, note], i) =>
                <div key={i} style={{ display: "flex", gap: 8, padding: "4px 0", borderBottom: `1px solid ${C.navyLt}` }}>
                        <span style={{ color: i === 0 ? C.red : i === 3 ? C.amber : C.green, fontWeight: 700, minWidth: 90, fontFamily: "monospace" }}>{range}</span>
                        <span style={{ color: C.grey }}>{note}</span>
                      </div>
                )}
                  </div>
                  {s4.pkmEnabled &&
              <div style={{ marginTop: 10, background: C.navyLt, borderRadius: 6, padding: "10px 14px" }}>
                  ...
                  map((row, i) => {
                    ...
                    return (
                      <tr key={i} style={{ borderBottom: `1px solid ${C.navyLt}`, background: i % 2 === 0 ? C.navyLt + "30" : "transparent" }}>
                            ...
                            <td style={{ padding: "6px 8px", color: C.white, fontFamily: "monospace" }}>{row.s3}</td>
                            <td style={{ padding: "6px 8px", color: row.up ? C.green : C.red, fontFamily: "monospace", fontWeight: 700 }}>
                              {row.s4} <span style={{ fontSize: 14 }}>{row.up ? "↑" : "↓"}</span>
                            </td>
                            <td style={{ padding: "6px 8px", color: row.up ? C.green : C.red, fontFamily: "monospace" }}>
                              {row.up ? "+" : ""}{delta}
                            </td>
                    ...
                    map((row, i) => {
                      ...
                      return (
                        <tr key={i} style={{ borderBottom: `1px solid ${C.navyLt}`, background: i % 2 === 0 ? C.navyLt + "30" : "transparent" }}>
                              ...
                              <td style={{ padding: "6px 8px", color: C.green, fontFamily: "monospace", fontWeight: 700 }}>{row.a}</td>
                              <td style={{ padding: "6px 8px", color: C.amber, fontFamily: "monospace", fontWeight: 700 }}>{row.b}</td>
                              <td style={{ padding: "6px 8px", color: diff > 0 ? C.amber : diff < 0 ? C.green : C.grey, fontFamily: "monospace" }}>
                                {diff >= 0 ? "+" : ""}{diff}
                              </td>
                            </tr>);
                  ...
                  map((r, i) => {
                    ...
                    return (
                      <tr key={i} style={{ borderBottom: `1px solid ${C.navyLt}`, background: i % 2 === 0 ? C.navyLt + "30" : "transparent" }}>
                          ...
                          <td style={{ padding: "5px 8px", color: C.green, fontFamily: "monospace", fontWeight: 700 }}>{r.fmt(r.a)}</td>
                          <td style={{ padding: "5px 8px", color: C.amber, fontFamily: "monospace", fontWeight: 700 }}>{r.fmt(r.b)}</td>
                          <td style={{ padding: "5px 8px", color: diffCol, fontFamily: "monospace" }}>{diff >= 0 ? "+" : ""}{diff}</td>
                        </tr>);
              ...
              <div style={S.card}>
                ...
                <div style={{ marginBottom: 10 }}>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    ...
                    <div key={t.k}
                    style={{ background: s4.frassCert === t.k ? t.col + "33" : C.navyLt, border: `1px solid ${s4.frassCert === t.k ? t.col : C.greyLt + "33"}`,
                    ...
                    {allSoilsNPK.map((row, i) => {
                      ...
                      return (
                        <tr key={i} style={{ borderBottom: `1px solid ${C.navyLt}`,
                          ...
                          border: isActive ? `1px solid ${C.teal}44` : undefined }}>
                          <td style={{ padding: "6px 8px", color: isActive ? C.teal : C.white, fontWeight: isActive ? 900 : 400 }}>
                            {row.name} {isActive && "← active"}
                          </td>
                          <td style={{ padding: "6px 8px", color: C.grey, fontFamily: "monospace" }}>{row.pct}</td>
                          <td style={{ padding: "6px 8px", color: C.grey, fontFamily: "monospace" }}>{row.ph}</td>
                          <td style={{ padding: "6px 8px", color: C.green, fontFamily: "monospace", fontWeight: 700 }}>${row.urea}/t FW</td>
                          <td style={{ padding: "6px 8px", color: C.teal, fontFamily: "monospace", fontWeight: 700 }}>${row.sm}/t FW</td>
                        </tr>);
                ...
                <div style={{ background: "#12201a", border: "1px solid #2a5030", borderRadius: 8, padding: "12px", marginTop: 10, marginBottom: 10 }}>
                      ...
                      {SOILS.map((soil, i) => {
                        ...
                        return (
                          <tr key={i} style={{ borderBottom: "1px solid #1a3025", background: isActive ? "#1a3a22" : "transparent" }}>
                            ...
                            <td style={{ padding: "5px 6px", color: "#7aa890", fontFamily: "monospace" }}>{soil.pct}</td>
                            <td style={{ padding: "5px 6px", fontFamily: "monospace", fontWeight: 800,
                            ...
                            <td style={{ padding: "5px 6px", fontFamily: "monospace", color: "#f0a030" }}>{baseline.toLocaleString()}</td>
                            <td style={{ padding: "5px 6px", fontFamily: "monospace", color: "#3dcb7a", fontWeight: 700 }}>{avoided.toLocaleString()}</td>
                            <td style={{ padding: "5px 6px", fontFamily: "monospace", color: "#f0a030", fontWeight: 800 }}>${rev.toLocaleString()}</td>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8 }}>
                    <div style={{ background: "#0e2818", borderRadius: 6, padding: "8px 10px", border: "1px solid #2a5030" }}>
                      ...
                      <div style={{ fontSize: 16, fontWeight: 900, color: "#8de8b4", fontFamily: "monospace" }}>{dmpp_N_kgday.toLocaleString()} kg N</div>
                    ...
                    <div style={{ background: "#0e2818", borderRadius: 6, padding: "8px 10px", border: "1px solid #2a5030" }}>
                      ...
                      <div style={{ fontSize: 16, fontWeight: 900, color: "#f0a030", fontFamily: "monospace" }}>{dmpp_co2e_base_ann.toLocaleString()} t CO₂e/yr</div>
                    ...
                    <div style={{ background: s0.dmppEnabled ? "#102818" : "#0e1a18", borderRadius: 6, padding: "8px 10px", border: s0.dmppEnabled ? "1px solid #3dcb7a44" : "1px solid #1a2820" }}>
                      ...
                      <div style={{ fontSize: 16, fontWeight: 900, color: s0.dmppEnabled ? "#3dcb7a" : "#445050", fontFamily: "monospace" }}>{dmpp_co2e_avoid_ann.toLocaleString()} t CO₂e/yr</div>
                    ...
                    <div style={{ background: s0.dmppEnabled ? "#1a3520" : "#0e1a18", borderRadius: 6, padding: "8px 10px", border: s0.dmppEnabled ? "1px solid #3dcb7a" : "1px solid #1a2820" }}>
                      ...
                      <div style={{ fontSize: 16, fontWeight: 900, color: s0.dmppEnabled && dmpp_roi >= 3 ? "#3dcb7a" : s0.dmppEnabled && dmpp_roi >= 2 ? "#f0a030" : "#445050", fontFamily: "monospace" }}>{s0.dmppEnabled ? dmpp_roi + "x" : "—"}</div>
                ...
                <div style={{ background: C.navyLt, borderRadius: 8, padding: "12px" }}>
                  ...
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "6px 8px",
                    background: C.navy, borderRadius: 4, marginBottom: 4 }}>
            ...
            {s4.pathwayS5 === "s5a" &&
              <div style={{ ...S.card, border: `1px solid ${C.teal}44`, marginTop: 0 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginBottom: 14 }}>
                  {/* BSF Insect Meal */}
                  <div style={{ background: C.navyLt, borderRadius: 8, padding: "14px", border: `1px solid ${C.teal}33` }}>
                  ...
                  {/* BSF Oil */}
                  <div style={{ background: C.navyLt, borderRadius: 8, padding: "14px", border: `1px solid ${C.amber}33` }}>
                    <div style={{ color: C.amber, fontWeight: 800, fontSize: 13, marginBottom: 6 }}>🛢️ BSF Insect Oil</div>
                  ...
                  {/* Chitin */}
                  <div style={{ background: C.navyLt, borderRadius: 8, padding: "14px", border: `1px solid ${C.green}33` }}>
            ...
            {s4.pathwayS5 === "s5b" &&
              <div style={{ ...S.card, border: `1px solid ${C.amber}44`, marginTop: 0 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginTop: 12 }}>
                  ...
                  { icon: "🛢️", label: "BSF Insect Oil", value: "Not produced", col: C.red },
                  { icon: "🦐", label: "BSF Chitin", value: "Not produced", col: C.red }].
                  map((p, i) =>
                  <div key={i} style={{ background: C.navyLt, borderRadius: 8, padding: "12px",
        ...
        <div>
            ...
            <div style={{ ...S.card, marginBottom: 16 }}>
              ...
              map((tier) =>
              <div key={tier.k}
              style={{ background: s4.certLevel === tier.k ? tier.col + "33" : C.navyLt,
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
              ...
              <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "6px 8px",
                ...
                <hr style={S.divider} />
                <div style={{ background: C.navyLt, borderRadius: 6, padding: "8px 10px", marginBottom: 8, fontSize: 11 }}>
              ...
              <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "8px 10px",
                background: C.navyLt, borderRadius: 6, marginBottom: 4 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              ...
              <div style={S.card}>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                  ...
                  map((r, i) =>
                  <tr key={i} style={{ borderBottom: `1px solid ${C.navyLt}`, background: r.bold ? C.navyLt : "transparent" }}>
              ...
              <div style={S.card}>
                ...
                return (
                  <div key={i} style={{ marginBottom: 6 }}>
                      ...
                      <div style={{ background: C.navyLt, borderRadius: 4, height: 8, overflow: "hidden" }}>
              ...
              <div style={S.card}>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                  ...
                  map((r, i) => {
                    const isBase = i === 0;
                    return (
                      <tr key={i} style={{ borderBottom: `1px solid ${C.navyLt}`, background: isBase ? C.navyLt : "transparent" }}>
                          <td style={{ padding: "7px 10px", color: r.col, fontWeight: isBase ? 900 : 400 }}>{r.scen}</td>
                          <td style={{ padding: "7px 10px", color: r.col, fontFamily: "monospace", fontWeight: isBase ? 900 : 400 }}>
                            {r.pb === null || r.pb === "N/A" ? "N/A" : r.pb + " yrs"}
                          </td>
                          <td style={{ padding: "7px 10px", color: r.npv > 0 ? C.green : C.red, fontFamily: "monospace", fontWeight: isBase ? 900 : 400 }}>
                            {r.npv > 0 ? "+" : ""}{typeof r.npv === "number" ? "$" + r.npv.toLocaleString() : "N/A"}
                          </td>
                          <td style={{ padding: "7px 10px", color: isBase ? C.grey : r.delta > 0 ? C.green : C.red, fontFamily: "monospace" }}>
                ...
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                    ...
                    <tr key={i} style={{ borderBottom: `1px solid ${C.navyLt}`, background: i % 2 === 0 ? C.navyLt + "30" : "transparent" }}>
                        ...
                        const dn = j > 0 && parseFloat(v) < parseFloat(prev);
                        return (
                          <td key={j} style={{ padding: "6px 8px", textAlign: "center", fontFamily: "monospace",
        ...
        {stage === 8 && (() => {
          ...
          const opdcFreshMo = +(monthFFB * 0.0456 * (s0.opdcCapturePct / 100)).toFixed(0);
          ...
          const BUYERS_CC = [
          { name: "Microsoft", logo: "🖥️", price: "$20–25/t", advance: "25–35%", tier: "primary", color: cc.tealLt, focus: "Agricultural methane avoidance", standard: "Verra VCS, CORSIA", fit: "Palm waste = agricultural residue" },
          ...
          { name: "Aviation CORSIA", logo: "✈️", price: "$15–30/t", advance: "Forward contracts", tier: "secondary", color: cc.greyLt, focus: "CORSIA compliance from 2027", standard: "CORSIA (ACM0022)", fit: "Demand surge 2027 — register now" },
            ...
            <div style={{ fontFamily: fm.body, fontSize: 12 }}>
              ...
              <div style={{ background: "#12201a", border: "1px solid #2a6040", borderRadius: 8, padding: 12, marginBottom: 10 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8, marginBottom: 10 }}>
                  ...
                  <div style={{ background: "#0e2818", borderRadius: 6, padding: "8px 10px", border: "1px solid #2a5030" }}>
                    ...
                    <div style={{ fontSize: 18, fontWeight: 900, color: "#8de8b4", fontFamily: "monospace" }}>{dmpp_N_kgday.toLocaleString()}</div>
                  ...
                  <div style={{ background: "#0e2818", borderRadius: 6, padding: "8px 10px", border: "1px solid #2a5030" }}>
                    ...
                    <div style={{ fontSize: 18, fontWeight: 900, color: "#f0a030", fontFamily: "monospace" }}>{dmpp_co2e_base_ann.toLocaleString()}</div>
                  ...
                  <div style={{ background: s0.dmppEnabled ? "#102a18" : "#0e1a18", borderRadius: 6, padding: "8px 10px", border: s0.dmppEnabled ? "1px solid #3dcb7a44" : "1px solid #2a3030" }}>
                    ...
                    <div style={{ fontSize: 18, fontWeight: 900, color: s0.dmppEnabled ? "#3dcb7a" : "#445050", fontFamily: "monospace" }}>{dmpp_co2e_avoid_ann.toLocaleString()}</div>
                  ...
                  <div style={{ background: s0.dmppEnabled ? "#1a3520" : "#0e1a18", borderRadius: 6, padding: "8px 10px", border: s0.dmppEnabled ? "1px solid #3dcb7a" : "1px solid #2a3030" }}>
                    ...
                    <div style={{ fontSize: 18, fontWeight: 900, color: s0.dmppEnabled ? "#3dcb7a" : "#445050", fontFamily: "monospace" }}>${s0.dmppEnabled ? dmpp_credit_rev_ann.toLocaleString() : "—"}</div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 10 }}>
                  ...
                  <div style={{ background: s0.dmppEnabled ? "#102818" : "#0e1a18", borderRadius: 6, padding: "8px 10px", border: s0.dmppEnabled ? "1px solid #3dcb7a44" : "1px solid #2a3030" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                      ...
                      <span style={{ fontSize: 10, fontWeight: 800, color: "#f0a030", fontFamily: "monospace" }}>${dmpp_cost_annual.toLocaleString()}</span>
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                      <span style={{ fontSize: 8, color: "#7aa890", fontWeight: 700, textTransform: "uppercase" }}>Net Gain/yr</span>
                      <span style={{ fontSize: 10, fontWeight: 800, color: dmpp_net_annual >= 0 ? "#3dcb7a" : "#e84040", fontFamily: "monospace" }}>${s0.dmppEnabled ? dmpp_net_annual.toLocaleString() : "—"}</span>
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      <span style={{ fontSize: 8, color: "#7aa890", fontWeight: 700, textTransform: "uppercase" }}>ROI</span>
                      <span style={{ fontSize: 14, fontWeight: 900, color: dmpp_roi >= 3 ? "#3dcb7a" : dmpp_roi >= 2 ? "#f0a030" : "#e84040", fontFamily: "monospace" }}>{s0.dmppEnabled ? dmpp_roi + "x" : "—"}</span>
                    </div>
                  </div>
                </div>
                    ...
                    {SOILS.map((soil, i) => {
                      ...
                      return (
                        <tr key={i} style={{ borderBottom: "1px solid #1a3025", background: isActive ? "#1a3a20" : "transparent" }}>
                          ...
                          <td style={{ padding: "4px 6px", color: "#7aa890", fontFamily: "monospace" }}>{soil.pct}</td>
                          <td style={{ padding: "4px 6px", fontFamily: "monospace", fontWeight: 700, color: eff >= 0.40 ? "#3dcb7a" : eff >= 0.30 ? "#f0a030" : "#8898b0" }}>{Math.round(eff * 100)}%</td>
                          <td style={{ padding: "4px 6px", fontFamily: "monospace", color: "#3dcb7a", fontWeight: 700 }}>{avoid.toLocaleString()}</td>
                          <td style={{ padding: "4px 6px", fontFamily: "monospace", color: "#f0a030", fontWeight: 700 }}>${rev.toLocaleString()}</td>
              ...
              <div>
                  <div style={cs.card}>
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 10, minWidth: 700 }}>
                      <thead>
                        <tr style={{ background: "#0f2236", borderBottom: "2px solid " + cc.tealLt + "55" }}>
              ...
              <div>
                  ...
                  <div style={cs.card}>
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 10 }}>
                      <thead>
                        <tr style={{ background: "#0f2236", borderBottom: "2px solid " + cc.tealLt + "44" }}>
              ...
              <div>
                  ...
                  <div style={cs.card}>
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 10 }}>
                      <thead><tr style={{ borderBottom: "1px solid #2a4070", background: "#0f2236" }}>{["Company", "Total", "Scope 3", "S3%", "Palm ID (t)", "CFI Opp."].map(function (h) {return React.createElement("th", { key: h, style: { ...cs.th, textAlign: "left", padding: "7px 8px" } }, h);})}</tr></thead>
                      <tbody>
        ...
        <div>
            <div style={S.card}>
              ...
              map((s) =>
              <div key={s.stage} style={{ background: C.navyLt, borderRadius: 8, padding: "14px",
              ...
              <div style={{ fontSize: 12, color: C.greyLt, lineHeight: 2.0 }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 20px" }}>
                  ...
                  <div><span style={{ color: C.red }}>Safety:</span> Bt ICBB 6095 {s3.bt_confirmed_absent ? "confirmed absent ✅" : "⚠️ not confirmed"} | NaOH {s2.naoh ? "⚠️ ACTIVE" : "safe"}</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <div style={{ textAlign: "center", color: C.grey, fontSize: 10, marginTop: 20, borderTop: `1px solid ${C.navyLt}`, paddingTop: 12 }}>
        CFI Bioconversion System · S0→S6 + Carbon Credits · All 31 Chemicals · All 40 Organisms · Bt NOW ALLOWED (timing-critical) · IPCC FOD + COD Carbon Model · Nutrient Ledger S0→S6 · Orchestration Layer · Core: EFB OPDC POME_sludge · Additives: PKE PMF fronds CPO · Supabase LIVE · v21.0
      </div>
    </div>);
