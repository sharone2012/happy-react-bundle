...
/* ââ KPI Strip ââ */
const kpis = [
  { label: "Total Building CAPEX", value: "$1,374,610", color: C.amber, unit: "S1 Building Only Â· Indo Rates", dot: C.teal2 },
  { label: "Total Equipment CAPEX", value: "$398,000", color: C.amber, unit: "POS decanter $80Kâ$150K RFQ pending", dot: C.teal2 },
  { label: "Monthly OPEX", value: "$37,957/mo", color: C.green, unit: "Labour + Electricity + Maintenance + Admin", dot: C.teal2 },
  { label: "Processing Lines", value: "3 Active", color: C.green, unit: "EFB Â· OPDC Â· POS", dot: C.teal2 },
  { label: "Facility Area", value: "2,450 mÂ²", unit: "Building footprint", color: C.teal2, dot: C.teal2 },
...
/* ââ Site & Facility Metrics Card ââ */
const visibleTiles = [
  { label: "Total Site Area", value: "5,000 mÂ²", unit: "1.25 acres recommended" },
  { label: "Building Footprint", value: "1,260 mÂ²", unit: "36m Ã 35m Ã 10m ht" },
];

const expandedTiles = [
  { label: "Outside Space", value: "2,540 mÂ²", unit: "Roads Â· silos Â· workshop" },
  { label: "Truck Bay", value: "216 mÂ²", unit: "18m Ã 12m Ã 4m ht" },
  { label: "Conv. Gallery", value: "100 mÂ²", unit: "25m Ã 4m Â· covered" },
  { label: "Greenhouse Block", value: "1,920 mÂ²", unit: "4 Ã 40Ã8m Â· separate S3" },
  { label: "EFB Machines", value: "10 units", unit: "â  â â© Â· 20 t/h" },
  { label: "OPDC Machines", value: "8 units", unit: "âª â â³ Â· 5 t/h" },
  { label: "Total Conveyors", value: "22", unit: "EFB 9 Â· OPDC 8 Â· Gallery 5" },
  { label: "Portal Frames", value: "7", unit: "@ 6m spacing Â· PEB" },
];

const drawingBtns = [
  { title: "Site Master Plan", sub: "98m Ã 85m Â· All zones Â· North arrow", action: "nav", route: "/s1-combined" },
  { title: "Building Architecture", sub: "Floor Â· Elevation Â· Section Â· 3D Â· Machinery", action: "toast" },
  { title: "Building Floor Plan", sub: "36Ã35m Â· All machines â ââ³ Â· Legend", action: "nav", route: "/s1-combined" },
  { title: "Greenhouse Design", sub: "Site plan Â· Section XX Â· Store", action: "toast" },
  { title: "EFB Floor Plan", sub: "â ââ© Â· 20 t/h Â· 10 machines Â· 600mm belt", action: "nav", route: "/s1-combined" },
  { title: "OPDC Floor Plan", sub: "âªââ³ Â· 5 t/h Â· CLASS A gate", action: "nav", route: "/s1-combined" },
  { title: "EFB ASCII Flow", sub: "Step-by-step Â· machines Â· specs Â· gates", action: "nav", route: "/s1-efb-ascii" },
  { title: "OPDC ASCII Flow", sub: "Step-by-step Â· CLASS A gate Â· 24h dwell", action: "nav", route: "/s1-opdc-ascii" },
  { title: "POS ASCII Flow", sub: "ICP-OES Fe gate Â· decanter Â· inclusion rate", action: "nav", route: "/s1-pos-ascii" },
];
...
function MetricTile({ label, value, unit }) {
  return (
    <div style={{
      background: C.black,
      border: "1px solid rgba(30,107,140,0.25)",
      borderRadius: 4,
      padding: "8px 10px",
      textAlign: "center",
      display: "flex",
  ...
  return (
    <div style={{ background: C.s1, border: `1.5px solid ${C.border}`, borderRadius: 8, overflow: "hidden" }}>
      <div onClick={() => setOpen(!open)} style={{
        background: C.s1, padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center",
        borderBottom: "1px solid rgba(30,107,140,0.2)", cursor: "pointer", userSelect: "none",
      }}>
      ...
      {isOpen && (
        <>
          ...
          <div onClick={() => setMoreOpen(!moreOpen)} style={{
            padding: "6px 14px", borderTop: "1px solid rgba(30,107,140,0.15)", display: "flex",
            justifyContent: "center", cursor: "pointer", fontFamily: F, fontSize: 9, fontWeight: 600, color: C.teal2,
          }}>
          ...
          <div style={{
            paddingTop: 10, paddingBottom: 2, paddingLeft: 14, fontFamily: F, fontSize: 9, fontWeight: 700,
            color: C.teal2, textTransform: "uppercase", letterSpacing: "0.1em",
          ...
          <div style={{ padding: "8px 14px 12px", display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
            {drawingBtns.map((b, i) => (
              ...
              >
                <span style={{ position: "absolute", top: 4, right: 6, fontFamily: F, fontSize: 11, color: "rgba(139,160,180,0.4)" }}>â</span>
  ...
  return (
    <div style={{ background: C.s1, border: `1.5px solid ${C.border}`, borderRadius: 8, overflow: "hidden" }}>
      <div onClick={() => setOpen(!open)} style={{
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <Pill letter="G" />
          <span style={{ fontFamily: F, fontSize: 13, fontWeight: 700, color: C.teal2 }}>Building CAPEX â S1 Only</span>
      ...
      {isOpen && (
        <>
          <div style={{
            ...
            <div>
              <div style={{ fontFamily: F, fontSize: 9, fontWeight: 700, color: C.grey, textTransform: "uppercase", letterSpacing: "0.07em" }}>Total To CFI â S1 Building</div>
          ...
          <div style={{ borderTop: "1px dashed rgba(30,107,140,0.3)", padding: "8px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ display: "flex", alignItems: "center" }}>
              <span style={{ fontFamily: F, fontSize: 9, fontWeight: 600, color: C.grey, width: 45 }}>â</span>
              <span style={{ fontFamily: F, fontSize: 11, fontWeight: 700, color: C.white, paddingLeft: 8 }}>Base subtotal A1âA8</span>
            </div>
          ...
          <div style={{ borderTop: `2px solid ${C.green}`, padding: "8px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ display: "flex", alignItems: "center" }}>
              <span style={{ fontFamily: F, fontSize: 9, fontWeight: 600, color: C.grey, width: 45 }}>â</span>
              <span style={{ fontFamily: F, fontSize: 12, fontWeight: 700, color: C.white, paddingLeft: 8 }}>Total To CFI â Building</span>
            </div>
          ...
          <div style={{
            ...
            + View Detailed CAPEX Schedule â 41 Line Items
          </div>
        </>
      )}
    </div>
...
/* ââ Equipment CAPEX Card (Column 3) ââ */
const efbEquip = [
...
const posEquip = [
  { code: "DEC-SLD-101", desc: "POS 3-Phase Decanter", val: "RFQ $80Kâ$150K", rfq: true },
];

const sharedEquip = [
  { code: "S-LIME-01", desc: "Limestone Storage and Dosing", val: "$6,000" },
  { code: "V-LOADER-ROW-A-01", desc: "Wheel Loader â Duty", val: "$85,000" },
  { code: "V-LOADER-ROW-A-02", desc: "Wheel Loader â Standby", val: "$85,000" },
];
  ...
  return (
    <div style={{ background: C.s1, border: `1.5px solid ${C.border}`, borderRadius: 8, overflow: "hidden" }}>
      <div onClick={() => setOpen(!open)} style={{
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <Pill letter="E" />
          <span style={{ fontFamily: F, fontSize: 13, fontWeight: 700, color: C.teal2 }}>Equipment CAPEX â S1 Only</span>
          <Tag text="Indonesian Rates" />
        </div>
        <Chevron open={isOpen} />
      </div>
      ...
      {isOpen && (
        <>
          <div style={{ background: "rgba(61,203,122,0.05)", borderBottom: "1px solid rgba(30,107,140,0.2)", padding: "10px 14px" }}>
            ...
            <div style={{ fontFamily: F, fontSize: 9, color: C.red, marginTop: 2 }}>POS decanter: RFQ $80Kâ$150K (not included)</div>
          </div>
          ...
          <div style={{
            ...
            + View Detailed Equipment Schedule â 18 Line Items
          </div>
        </>
      )}
    </div>
    ...
    <div style={{ background: C.s1, border: `1.5px solid ${C.border}`, borderRadius: 8, overflow: "hidden", marginTop: 10 }}>
      <div onClick={() => setOpen(!open)} style={{
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <span style={{ fontFamily: F, fontSize: 13, fontWeight: 700, color: C.amber }}>â $37,957/mo</span>
        </div>
        <Chevron open={isOpen} />
      </div>
      ...
      {isOpen && (
        <div style={{ padding: "10px 14px", display: "flex", gap: 10 }}>
          {/* Labour */}
          <OpexSubTable
            title="Labour" total="$3,576/mo" subtitle="7 headcount"
          ...
          {/* Electricity */}
          <OpexSubTable
            title="Electricity" total="$22,648/mo" subtitle="PLN I-3 tariff Â· IDR 1,444.70/kWh"
          ...
          {/* Maintenance */}
          <OpexSubTable
            title="Maintenance and Other" total="$11,733/mo"
...
export default function S1CapexOpex() {
  ...
  return (
    <div style={{ fontFamily: F, color: C.white, background: C.bg, minHeight: "100vh", filter: "brightness(1.4)" }}>
      <style>{`@keyframes cfi-pulse { 0%{opacity:1} 50%{opacity:0.3} 100%{opacity:1} }`}</style>

      {/* ââ GLOBAL HEADER ââ */}
      <div style={{
        background: "#0A1628", height: 83, display: "flex", alignItems: "center", padding: "0 32px",
        borderBottom: "1px solid rgba(51, 212, 188, 0.15)", position: "fixed", top: 0, left: 0, right: 0, zIndex: 100,
      }}>
      ...
      {/* ââ CONTEXT BAR / BREADCRUMB ââ */}
      <div style={{
        background: "#1C2C34", borderBottom: "1px solid rgba(30,107,140,0.25)",
        ...
        <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
          <a href="/" style={{ color: C.teal2, fontWeight: 700, textDecoration: "none", fontFamily: F, fontSize: 9 }}
            onMouseEnter={e => e.currentTarget.style.opacity = 0.7}
            onMouseLeave={e => e.currentTarget.style.opacity = 1}>CFI Platform</a>
          <span style={{ color: "rgba(30,107,140,0.5)" }}> âº </span>
          <a href="/" style={{ color: C.grey, textDecoration: "none", fontFamily: F, fontSize: 9 }}
            onMouseEnter={e => e.currentTarget.style.opacity = 0.7}
            onMouseLeave={e => e.currentTarget.style.opacity = 1}>S0 Mill Config</a>
          <span style={{ color: "rgba(30,107,140,0.5)" }}> âº </span>
          <span style={{ color: C.teal2, fontWeight: 700 }}>S1 Pre-Processing</span>
        </div>
        <span style={{ fontFamily: F, fontSize: 9, color: C.grey }}>S1 Rev 01 Â· March 2026</span>
      </div>

      {/* ââ PAGE HEADER â title + mill info on same row ââ */}
      <div style={{
        background: "#1C2C34", borderBottom: "1px solid rgba(30,107,140,0.2)",
        padding: "11px 20px", display: "flex", justifyContent: "space-between", alignItems: "center",
      }}>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <Pill letter="S1" />
          <span style={{ fontFamily: F, fontSize: 16, fontWeight: 700, color: C.white, marginLeft: 6 }}>
            Pre-Processing â CAPEX Â· OPEX Â· Facility
          </span>
          {/* Mill info â now on the same row */}
          <span style={{ color: "rgba(30,107,140,0.5)", margin: "0 6px", fontSize: 14 }}>|</span>
          <span style={{ fontFamily: F, fontSize: 11, color: C.grey }}>Mill: </span>
          <span style={{ fontFamily: F, fontSize: 11, fontWeight: 700, color: C.teal2 }}>Bogor Test Mill</span>
          <span style={{ color: "rgba(30,107,140,0.5)", margin: "0 2px" }}> Â· </span>
          <span style={{ fontFamily: F, fontSize: 11, color: C.grey }}>FFB: </span>
          <span style={{ fontFamily: F, fontSize: 11, fontWeight: 700, color: C.teal2 }}>60 TPH</span>
          <span style={{ color: "rgba(30,107,140,0.5)", margin: "0 2px" }}> Â· </span>
          <span style={{ fontFamily: F, fontSize: 11, color: C.grey }}>Active: </span>
          <div style={{ display: "flex", gap: 6 }}>
            <Tag text="EFB" />
            <Tag text="OPDC" />
            <Tag text="POS â ICP-OES Pending" color={C.red} />
          </div>
        </div>
        <div style={{ display: "flex", gap: 5, flexShrink: 0 }}>
          {[
            { label: "â Site Plan", c: "rgba(0,201,177,", tc: C.teal, onClick: () => navigate("/s1-combined") },
            { label: "â Architecture", c: "rgba(155,89,182,", tc: "#c090ff", onClick: () => toast("Architecture view coming soon", { duration: 2000 }) },
